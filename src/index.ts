const printLine = (text: string, breakLine: boolean = true): void => {
...
class HitAndBrow implements Game {
	...
	async play() {
		const inputString = await promptInput(`「,」区切りで${this.getAnswerLength()}つの数字を入力してください`)
	...
	private validate(inputArr: string[]): boolean {
		...
		const isAllAnswerSourceOption = inputArr.every((value) => this.answerSource.includes(value))
		const isAllDifferentValues = inputArr.every((value, index) =>
			inputArr.indexOf(value) === index
		)
